...
import cn.library.pojo.em.LoginStatusEnum;
import cn.library.pojo.em.RoleEnum;
import cn.library.pojo.em.WordStatusEnum;
...
public class UserServiceImpl implements UserService {
    ...
    public Result<String> updatePwd(Map<String, String> map) {
        ...
        if (Objects.isNull(oldPwd)) {
            return ApiResult.error("The original password input cannot be empty");
        }
        if (Objects.isNull(newPwd)) {
            return ApiResult.error("Please enter your new password");
        }
        if (Objects.isNull(againPwd)) {
            return ApiResult.error("Please confirm your password");
        }
        if (!newPwd.equals(againPwd)) {
            return ApiResult.error("The passwords entered before and after are inconsistent");
        }
        User user = userMapper.getByActive(
